...
posDF <- miceTable[miceTable$delta_ct_MminusE > 0,]
...
ggplot(posDF, aes(delta_ct_MminusE)) +
...
paramBounds = c(L1start = mean(miceTable$delta_ct_MminusE), 
                ...
                myshapestart = fits$weibull$estimate[["shape"]], myshapeLB = .5, myshapeUB = 100)

fitWeibull <- analyse(miceTable, "delta_ct_MminusE", paramBounds = paramBounds)
