...
FitBasicNoAlpha <- function(data, response, hybridIndex, paramBounds, config){
  ...
  fit <- mle2(
    response ~ dweibull(shape = 1.44, scale = MeanLoad(L1, L1, 0, HI)),
    data = data, 
    start = start,
...
FitBasicAlpha <- function(data, response, hybridIndex, paramBounds, config){
  ...
  fit <- mle2(
    response ~ dweibull(shape = 1.44, scale = MeanLoad(L1, L1, alpha, HI)),
    data = data,
    start = start,
    lower = c(L1 = paramBounds[["L1LB"]],
...
FitAdvancedNoAlpha <- function(data, response, hybridIndex, paramBounds, config){
  ...
  fit <- mle2(
    response ~ dweibull(shape = 1.44, scale = MeanLoad(L1, L2, 0, HI)),
    data = data, 
    start = start,
    lower = c(L1 = paramBounds[["L1LB"]], 
              L2 = paramBounds[["L2LB"]]              ),
...
FitAdvancedAlpha <- function(data, response, hybridIndex, paramBounds, config){
  ...
  start <-  list(L1 = paramBounds[["L1start"]],
                 ...
                 alpha = paramBounds[["alphaStart"]])
  fit <- mle2(
    response ~ dweibull(shape = 1.44, scale = MeanLoad(L1, L2, alpha, HI)),
    data = data, 
    start = start,
    lower = c(L1 = paramBounds[["L1LB"]], 
              L2 = paramBounds[["L2LB"]], 
