...
TestData <- list(males = SimulatedData(50, 10, 1, 2, 2), 
                 females = SimulatedData(50, 10, 9, 2, 2))
...
#PrNegativeBinomialLoad: Probability an Ind has some observed Load, given its HI (and the model)    
...
```{r}
#Formatting TestData for gg_point
TestDataDF_M <- as.data.frame(TestData$males)
TestDataDF_M$sex <- "males"
TestDataDF_F <- as.data.frame(TestData$females)
TestDataDF_F$sex <- "females"
TestDataDF <- rbind(TestDataDF_F, TestDataDF_M)
...
plot0 <- ggplot(df, aes(x=HI, y=est))+
    ggtitle("H0")+
    geom_point(data = TestDataDF, aes(x=IndHIs, y=IndLoads), color= "green", size=0.3)+
    geom_line()+
...
plot2 <-ggplot(df, aes(x=HI, y=estM))+
    ggtitle("H2")+
    geom_point(data = TestDataDF, aes(x=IndHIs, y=IndLoads, group=sex, color=sex), size=0.3)+
...
```{r}
M <- multiplot(plot0,plot1,plot2,plot3, cols=2)
M
```
